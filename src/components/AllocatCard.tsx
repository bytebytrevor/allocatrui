...
export function AllocatCardGrid({
  ...
  function getInviteButtonClass(compact = false) {
    const base = compact
      ? "h-8 w-8 rounded-lg p-0 text-[0.65rem] font-semibold shadow-none sm:h-9 sm:w-auto sm:px-3 lg:px-4 lg:text-xs"
      : "h-10 flex-1 rounded-lg text-xs font-semibold shadow-none";

    if (isAccepted) {
      return `${base} border-chart-2/25 bg-chart-2/[0.08] text-chart-2 hover:bg-chart-2/[0.12] hover:text-chart-2`;
    }

    if (isInvited) {
      return `${base} border-chart-3/25 bg-chart-3/[0.08] text-chart-3 hover:bg-chart-3/[0.12] hover:text-chart-3`;
    }
            ...
            <div className="flex shrink-0 items-center gap-1 sm:gap-2">
              ...
              {isDiscovery ? (
                <Button
                  ...
                  aria-label={`Start a project with ${allocat.fullName || "this Allocat"}`}
...
function ProfileAvatar({
  ...
  return (
    <div className="relative shrink-0">
      <Avatar
        className={`${avatarSize} border border-border bg-muted`}
      >
        {allocat.avatarUrl && (
          <AvatarImage
  ...
  if (rating <= 0) {
    return (
      ...
      >
        ...
        <span className="text-[0.65rem] font-bold lg:text-[0.7rem]">
          New
        </span>
      </span>
    );
